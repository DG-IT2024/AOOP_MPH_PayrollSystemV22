...
public class MotorPH_fileRead {
    ...
    public static void main(String[] args) {
        ...
        while (true) {
            ...
            String source;
            do {
                System.out.println("\nHow do you like put your entry?");
                System.out.println("A. Upload 'input.txt' file Located in the Program Folder");
                System.out.println("B. Type on Console");
                source = entry.nextLine().trim().toLowerCase();
                if (!source.equals("a") && !source.equals("b")) {
                    System.out.println("Invalid input. Please choose from the available source.");
                }
            } while (!source.equals("a") && !source.equals("b"));

            if (source.equalsIgnoreCase("a")) {
                fileReadDataEntry(coveredDays, maxRegularHours);
    ...
    public static int fileReadDataEntry(int coveredDays, int maxRegularHours) {

        System.out.println("extracting data from 'input.txt'...");

        String filepath = "input.txt"; //text file to be read

        ArrayList<String> inputs = extractData(filepath);

        //Extract LastName and First Name. Use as search criterion
        String employeeNumber;
        employeeNumber = inputs.get(2);
    ...
    public static ArrayList<String> extractData(String filePath) {
        ...
        try {
            Scanner extractDataScan = new Scanner(filePath_);

            ArrayList<String> inputs = new ArrayList<>();

            while (extractDataScan.hasNextLine()) {
                String inputline = extractDataScan.nextLine(); //before "\n" newline charter
